...
(function () {
    ...
    var q = /\$(?:(\d\d?|[$&`'])|{([$\w]+)})/g, D = /[^gimy]+|([\s\S])(?=[\s\S]*\1)/g,
        w = /^(?:[?*+]|{\d+(?:,\d*)?})\??/, A = false, v = [], o = {
        ...
        }, F = o.exec.call(/()??/, "")[1] === undefined, x = function () {
            var f = /^/g;
            o.test.call(f, "");
            return !f.lastIndex
        }(), G = function () {
    ...
    var sh = {
        ...
        toolbar: {
            ...
            getHtml: function (highlighter) {
                var html = '<div class="toolbar"><div class="row">',
            ...
            items: {
                ...
                expandSource: {
                    getHtml: function (highlighter) {
                        ...
                        return '<div class="col-md-1"><a id="" class="toolbar-item open-code"><span class="glyphicon glyphicon-plus"></span><span class="item-desc">展开代码</span></a></div>';
                        // return sh.toolbar.getButtonHtml(highlighter, 'expandSource', title ? title : sh.config.strings.expandSource);
                    },
                    execute: function (highlighter) {
                        var div = getHighlighterDivById(highlighter.id);
                ...
                copySource: {
                    getHtml: function (highlighter) {
                        ...
                        return '<div class="col-md-1 col-md-offset-9"><a id="" class="toolbar-item copy-source"><span class="glyphicon glyphicon-copy" ></span>复制</a></div>';
                        // return sh.toolbar.getButtonHtml(highlighter, 'expandSource', title ? title : sh.config.strings.expandSource);
                    }
                },
            }
        ...
        highlighterCode: function (orgCode, language) {
            ...
            var brush = null, conf = sh.config;
            var brushName = language, code;
            brush = findBrush(brushName);
            if (brush) brush = new brush();
            else return orgCode;
            if (conf.useScriptTags) code = stripCData(orgCode);
            var params = {};
            params['brush'] = brushName;
            brush.init(params);
            //代码内容过多时，开启折叠
            if (code.toString().length > 1000){
                brush.params['collapse'] = true;
                brush.params['toolbar'] = true;
            }else{
                brush.params['toolbar'] = false;
            }
            var element = brush.getDiv(code);
            var textareaElem = document.createElement('textarea');
            var codeElem = document.createTextNode(code.toString());
            textareaElem.appendChild(codeElem);
            textareaElem.style.display="none";
            element.appendChild(textareaElem);
            textareaElem.id = getHighlighterId(brush.id) + "_code";
        ...
        highlighterElem: function (orgCode, language) {
            ...
            textareaElem.appendChild(codeElem);
            textareaElem.style.display="none";
            textareaElem.id = getHighlighterId(brush.id.toString()) + "_code";
            element.appendChild(textareaElem);
            return element;
        }
    }; // end of sh
